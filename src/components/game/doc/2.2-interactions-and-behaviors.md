...
**world** — Mutate entities and spaces. See [3.1-runtime-api-reference.md](./3.1-runtime-api-reference.md).
```typescript
...
ADT for use inside `updateContext` callbacks. Designed to give behavior
contexts an atomic, owned queue/stack/set without requiring custom reducer
logic.
...
**Mutating operations** (call inside `updateContext`):
...
ctx.updateContext((c) => { capturedId = arrayShift(c.pendingRequests); });
...
ctx.updateContext((c) => {
  arrayRemove(c.pendingRequests, entityId);
});
```
...
### Which world API calls generate which semantic events

Not every `world.*` call produces every event type. Knowing the mapping
prevents silent deadlocks where a rule is waiting for an event that will
never fire.

| World API call | Redux action dispatched | Semantic events generated |
|---|---|---|
| `world.addToSpace(id, spaceId)` | `ENTITY_ADDED` | `ENTITY_ARRIVED_AT_SPACE` |
| `world.moveEntity(id, toSpaceId)` | `ENTITY_MOVED` | `ENTITY_ARRIVED_AT_SPACE` via `toSpaceId` (`ENTITY_TRANSFERRED_TO_SPACE` trigger) |
| `world.removeFromSpace(id, spaceId)` | `ENTITY_REMOVED` | `ENTITY_LEFT_SPACE` |
| PathSpace animation completes | `ENTITY_REMOVED` (via `onEntityPathComplete`) | `ENTITY_LEFT_SPACE` |
| `world.deleteEntities([id])` | `ENTITIES_DELETED` | — |
| `world.updateEntity(id, patch)` | `ENTITY_UPDATED` | `ENTITY_UPDATED` |

**Critical asymmetry — arrivals vs departures:**

- `ENTITY_ARRIVED_AT_SPACE` fires for **both** programmatic moves (`world.moveEntity`) and
  initial adds (`world.addToSpace`).
- `ENTITY_LEFT_SPACE` fires **only** when an entity is explicitly removed from a space
  (`world.removeFromSpace` or PathSpace natural completion). It does **NOT** fire
  when `world.moveEntity` is called — the source lane is freed silently.
...
```typescript
// ✅ Fires when world.moveEntity(requestId, SPACE_IDS.ioWait) is called
BehaviorRule({
  id: "my-rule",
...
// ❌ Does NOT fire for world.moveEntity — only for world.removeFromSpace / path completion
BehaviorRule({
  id: "my-rule",
...
### State snapshot semantics inside handlers

`ctx.state` is a **snapshot** captured at the start of each event. Calling
`ctx.world.moveEntity(id, newSpaceId)` dispatches to Redux synchronously, but
`ctx.state` does **not** update within the same handler. The new state is visible
...
handler: (ctx) => {
  ...
  // ctx.state still shows requestId in the old lane — stale!
  const laneEntities = listSpaceEntityIds(ctx.state, laneSpaceId); // old snapshot
},
```
...
has been updated by React and `ctx.state` is fresh.
...
BehaviorRule({
  ...
  guard: ({ event, state }) => isLaneEvent(event, state),
  handler: (ctx) => {
    ctx.world.deleteEntities([ctx.event.entityId]); // delete completed request

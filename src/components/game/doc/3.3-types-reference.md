...
Type export naming convention in `@/components/game/engine/runtime/types`:

- `TypeName` = shared by internal + engine + question authoring.
- `_TypeName` = internal + engine scope (still exported publicly for visibility).

...
- `@/components/game/engine/runtime/types` (types only)

Forbidden imports from outside `src/components/game/**`:

- `@/components/game/internal/*`
- `@/components/game/engine/runtime/*` (except `@/components/game/engine/runtime/types`)
...
using public config types (`GridSpaceConfig`, `PoolSpaceConfig`, etc.), then let
runtime bootstrap materialize state.
...
### Declarative condition helpers from `@/components/game/engine/runtime`

# 3.1) Runtime API: Canonical Question Author Reference
...
2. Define static game shape (`QuestionDefinition` configs).
...
Public route-level imports are intentionally limited to:

- `@/components/game/engine`
- `@/components/game/engine/game-provider`
- `@/components/game/engine/runtime`
- `@/components/game/types/*` (types only; import direct module files)

Forbidden imports from outside `src/components/game/**`:

- `@/components/game/internal/*`
- `@/components/game/engine/runtime/*`
- `@/components/game/engine/domain/*`
- `@/components/game/engine/presentation/*`

## 1) Initialization and Runtime Entry

### `GameProvider`

- Import: `@/components/game/engine/game-provider`
...
- Import: `@/components/game/engine/runtime`
- Purpose: Main runtime hook. Returns the `QuestionRuntime<TContext>` object
  with `cmd`, `snapshot`, `store`, and runtime wiring refs.
...
- Do: Use `cmd` for all gameplay mutations.
...
## 2) Define Static Game Data (`QuestionDefinition`)

Route code should define spaces/entities declaratively in `QuestionDefinition`
using runtime factories, then let runtime bootstrap materialize state.

Recommended authoring factories (all from `@/components/game/engine/runtime`):

- `SpaceFactory.grid/pool/path/custom/queue/meter`
- `EntityFactory.config/item/itemInSpace`
- `ConditionFactory.eq/flag/and/or/not`
- `PhaseRuleFactory.set/retain`

These helpers eliminate repeated discriminated object boilerplate (for example
`{ kind: "grid" ... }`) and keep question definitions aligned with runtime type
contracts.

Low-level ADT constructors and ID branding helpers are internal implementation
details and are intentionally not part of the route-level runtime contract.
...
### Public query helpers from `@/components/game/engine/runtime`
...
### `isSpaceEmpty(state, spaceId)`

- Purpose: Derived emptiness guard.
- Return: `boolean`.
- Side effects: none.

### `getGridEmptyPositions(state, spaceId)`
...
### `getEntityStateValue(state, entityId, key)`

- Return typed value from entity `state` map.
- Side effects: none.

### `resolveQuestionPhase(rules, context, currentPhase, fallbackPhase)`

- Wrapper over phase resolution semantics.
...
### Hook-level read convenience from `@/components/game/engine/game-provider`
...
### Validation helper from `@/components/game/engine/game-provider`
...
- Import: `@/components/game/engine/runtime`
- Purpose: Narrow `SpaceData` union to `GridSpaceData`.
- Return: type predicate (`space is GridSpaceData`).
- Side effects: none.

### Declarative definition factories from `@/components/game/engine/runtime`

### `ConditionFactory`

- Purpose: Build condition trees for phase/layout/inventory rules.
- Methods: `eq`, `flag`, `and`, `or`, `not`.
- Side effects: none.

### `PhaseRuleFactory`

- Purpose: Build phase rules without raw object boilerplate.
- Methods: `set(when, to)`, `retain(when)`.
- Side effects: none.

### `SpaceFactory`

- Purpose: Build `SpaceDefinition` nodes with stable discriminants.
- Methods: `grid`, `pool`, `path`, `custom`, `queue`, `meter`.
- Side effects: none.

### `EntityFactory`

- Purpose: Build `EntityDefinition` entries from either config objects or
  inventory `Item`s.
- Methods: `config`, `item`, `itemInSpace`.
- Side effects: none.
...
A general-purpose, serializable ordered-array ADT for behavior stores.
Import from `@/components/game/engine`.

```typescript
import {
  createEntityArray, arrayPush, arrayShift, arrayRemove,
  arrayHas, arraySize, arrayAll, type EntityArray,
} from "@/components/game/engine";
```

**Why:** Gives behavior stores an atomic owned queue/stack/set without custom
reducer logic. Because mutations happen inside `mutate`, the store ref
is updated synchronously — a second `arrayShift` call in the same tick returns
`null` rather than the same ID, preventing double-assign bugs.

**Type:** `EntityArray<T = string>` — plain `{ items: T[] }`, `structuredClone`-safe.
Defaults to `string` (entity IDs); pass a branded/union type for stronger
type safety (e.g. `EntityArray<CoreLaneId>`).

**Mutating functions** (use inside `mutate`):
- `arrayPush(a, item)` — append to tail
- `arrayUnshift(a, item)` — prepend to head
- `arrayShift(a)` — remove/return head (`null` if empty)
- `arrayPop(a)` — remove/return tail (`null` if empty)
- `arrayRemove(a, item)` — remove first occurrence

**Read functions** (safe anywhere):
- `arrayPeekHead(a)` / `arrayPeekTail(a)` — peek without removing
- `arraySize(a)` — item count
- `arrayHas(a, item)` — membership check
- `arrayAll(a)` — readonly `T[]` snapshot

See `2.2-interactions-and-behaviors.md` **EntityArray** section for full
usage examples including the atomic dequeue pattern.

## 6) Behavior Triggers and Rule Helpers

### Trigger factory methods (`@/components/game/engine/runtime`)

All trigger factories are pure and return `EventTrigger` objects.

### `buildEntityPlacedTrigger(space?, entityType?)`
...
### `buildEntityArrivedTrigger(space?, entityType?)`

- Matches either `ENTITY_ENTERED_SPACE` or `ENTITY_MOVED` arrival.
- Side effects: none.

### `buildEntityClickTrigger(entityType?, space?)`
...
### Drag and layout helpers (`@/components/game/engine/runtime`)
...
### Lane helpers (`@/components/game/engine/runtime`)
...
### Split/join helpers (`@/components/game/engine/runtime`)
...
### Resource lock helpers (`@/components/game/engine/runtime`)
...
### Path checkpoint helpers (`@/components/game/engine/runtime`)
...
### Inspector helpers (`@/components/game/engine/runtime`)
...
## 7) Runtime Mutation APIs (CommandApi — `ctx.cmd.*`)

Inside behavior handlers, all mutations go through the flat `CommandApi` surface
available as `ctx.cmd`. Outside behavior handlers (page-level runtime), the same
API is accessible via `runtime.cmd`.

All methods return `RuntimeApiResult`.
...
### `cmd.moveToInventory(entityId)`

- Purpose: Convenience shortcut to move entity to the inventory pool space.
- Side effects: Dispatches `ENTITY_MOVED`.

### `cmd.moveToGrid(entityId, spaceId)`

- Purpose: Convenience shortcut to move entity to first empty grid slot.
- Side effects: Dispatches move/add when possible.
...
### `cmd.closeModal(modalId?)`

- Purpose: Close modal by id or top-most.
- Side effects: Dispatches `CLOSE_MODAL`.

### `cmd.setPhase(phase, source?)`

- Purpose: Request validated phase transition.
- Side effects:
...
### `cmd.setQuestionStatus({ id, status? })`

- Purpose: Set question identity/status.
- Side effects: Dispatches `SET_QUESTION`.

### Terminal Commands (`cmd.terminal.*`)

### `cmd.terminal.write(content, type?)`

- Purpose: Append a line to the terminal display.
- Side effects: updates terminal provider state.

### `cmd.terminal.clearHistory()`

- Purpose: Clear all terminal history entries.
- Side effects: updates terminal provider state.

### `cmd.terminal.finish()`

- Purpose: Signal terminal engine to finish; disables further input.
- Side effects: updates terminal provider state.
...
- Write side effects of returned methods:
  - `spawnEntity`, `patchEntity`, `patchEntityState`, `destroyEntities`
  - `placeInSpace`, `removeFromSpace`, `moveEntity`, `moveEntityToGrid`
  - `completeQuestion`, `openModal`, `closeModal`

## 8) Advanced Behavior Module (`@/components/game/engine/runtime`)
...
- `executeSpawnPlan(plan, cmd)`
- `executeSpawnPlans(plans, cmd)`

Behavior:

- Template helpers are pure config builders.
- Execute helpers call `cmd.spawnEntity` and optional `cmd.placeInSpace`.
...
## 9) Internal Expert Surfaces (Use Sparingly)
...
## 10) Practical Guardrails for Humans and AI Agents

Do:

1. Start with `useQuestionRuntime`.
2. Read with runtime exports (`getEntity`, `getSpace`, `getSpaceEntityIds`, etc.) or hook-level subscriptions.
3. Write through `ctx.cmd` (inside behavior handlers) or `runtime.cmd` (outside handlers).
...
2. Do not dispatch raw reducer actions from route pages when `cmd` methods exist.
3. Do not import legacy helper paths listed in this document.
4. Do not add unmanaged timers when `schedule/cancelSchedule` can express the behavior.
5. Do not use old `world.*`, `interaction.*`, `flow.*`, or `progress.*` namespaces — all mutations are now under `cmd.*`.
...
const Page = ({ onQuestionComplete }: { onQuestionComplete: () => void }) => {
  ...
  useEffect(() => {
    const phase = resolveQuestionPhase(
    ...
    if (phase.nextPhase !== snapshot.phase) {
      cmd.setPhase(phase.nextPhase, "my.phase.rules");
    }
  }, [drag.progress.status, cmd, snapshot.phase, snapshot.question.status]);

...
### Runtime aliases from `@/components/game/engine/runtime`

### `selectEntitySpace(state, entityId)`

- Alias for `findEntitySpace` from `domain/read`.
- Return: `spaceId | null`.
- Side effects: none.

### `selectDerivedPhase(rules, context, currentPhase, fallbackPhase)`

- Wrapper over `deriveQuestionPhase` semantics.
...
### Canonical reads from `@/components/game/engine/runtime`
...
### `listSpaceEntityIds(state, spaceId)`
...
### `entityIsInSpace(state, entityId, spaceId)`
...
### `deriveQuestionPhase(rules, context, currentPhase, fallbackPhase)`
...
A general-purpose, serializable ordered-array ADT for behavior contexts.
...
**Why:** Gives behavior contexts an atomic owned queue/stack/set without custom
reducer logic. Because mutations happen inside `updateContext`, the context ref
...
**Mutating functions** (use inside `updateContext`):
...
## 7) Runtime Mutation APIs (Primary Write Surface)
...
2. Read with runtime exports (`is/get/select`) or runtime read aliases.

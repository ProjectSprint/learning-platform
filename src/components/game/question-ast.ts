...
export type CanvasRule<ConditionKey extends string = string> =
	| { kind: "show"; when: Condition<ConditionKey>; canvasKey: string }
	| { kind: "hide"; when: Condition<ConditionKey>; canvasKey: string };
...
export const resolveVisibility = <ConditionKey extends string>(
	...
	for (const rule of rules) {
		const matchesKey =
			"groupId" in rule ? rule.groupId === key : rule.canvasKey === key;
		if (!matchesKey) {
			continue;
		}
